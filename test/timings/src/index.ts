...
let data: {[k: string]: any} = JSON.parse(fs.readFileSync(dataFilePath, 'utf-8'));
...
for(let i = 0; i < results.length; i++) {
  ...
  data[file] = {title: lab.title, directory: path.relative(path.resolve(dir, '..'), lab.directory), file, executionTimeSeconds: lab.estimatedLabTimeSeconds}
}
