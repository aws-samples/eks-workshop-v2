...
:::note
You may skip this lab if you are running IPv6 cluster. Amazon EKS support for IPv6 primary goal is to solve IPv4 exhaustion issues. Hence custom networking doesn't apply to IPv6 clusters. By default, Amamzon VPC CNI is deployed in "PREFIX MODE" on IPv6 clusters. Amazon VPC CNI configure /80 IPv6 prefix during the node startup time. The CNI supports both IPv4 and IPv6 egress from within the Pods. Security groups for pods is not supported on IPv6 clusters yet.
:::
