...
다음 kustomization은 ConfigMap을 덮어쓰며, SDK가 테스트 Pod 대신 실제 DynamoDB 서비스를 기본값으로 사용하도록 지시하는 DynamoDB 엔드포인트 구성을 제거합니다. 또한 환경 변수 `RETAIL_CART_PERSISTENCE_DYNAMODB_TABLE_NAME`에서 가져온, 이미 생성된 DynamoDB 테이블의 이름을 제공했습니다.
