---
title: "Amazon DynamoDB 사용"
sidebar_position: 32
tmdTranslationSourceHash: '513c401e03117c6fbba61dec1d1d081f'
---

이 프로세스의 첫 번째 단계는 이미 생성된 DynamoDB 테이블을 사용하도록 carts 서비스를 재구성하는 것입니다. 애플리케이션은 대부분의 구성을 ConfigMap에서 로드합니다. 확인해 보겠습니다:
...
또한, 브라우저를 사용하여 애플리케이션의 현재 상태를 확인하세요. `ui` 네임스페이스에 `ui-nlb`라는 이름의 `LoadBalancer` 타입 서비스가 프로비저닝되어 있으며, 이를 통해 애플리케이션의 UI에 액세스할 수 있습니다.
...
위 명령에서 생성된 URL을 사용하여 브라우저에서 UI를 여십시오. 아래와 같이 Retail Store가 열려야 합니다.

![Home](/img/sample-app-screens/home.webp)

다음 kustomization은 ConfigMap을 덮어쓰고 DynamoDB 엔드포인트 구성을 제거합니다. 이는 SDK가 테스트 Pod 대신 실제 DynamoDB 서비스를 사용하도록 지시합니다. 또한 이미 생성된 DynamoDB 테이블 이름을 구성했습니다. 테이블 이름은 환경 변수 `RETAIL_CART_PERSISTENCE_DYNAMODB_TABLE_NAME`에서 가져옵니다.
...
`CARTS_DYNAMODB_TABLENAME`의 값을 확인한 다음 Kustomize를 실행하여 실제 DynamoDB 서비스를 사용하도록 하겠습니다:
...
이렇게 하면 ConfigMap이 새 값으로 덮어씌워집니다:
...
이제 새 ConfigMap 내용을 적용하기 위해 모든 carts Pod를 재시작해야 합니다:
