...
가용 영역(AZ) 장애의 전체 영향을 확인하기 위해 먼저 AZ당 두 개의 인스턴스로 확장하고 Pod 수를 9개로 늘려보겠습니다:
...
1. 먼저 Canary 아티팩트를 위한 S3 버킷을 생성합니다:
...
3. Cloudwatch 알람이 포함된 Synthetic Canary를 생성합니다:
...
이렇게 하면 매분마다 애플리케이션의 상태를 확인하는 Canary와 성공률이 95% 아래로 떨어지면 트리거되는 CloudWatch 알람이 설정됩니다.

이러한 단계가 완료되면 애플리케이션이 AZ의 두 인스턴스에 확장되었으며 다가오는 AZ 장애 시뮬레이션 실험에 필요한 모니터링이 설정되었습니다.
