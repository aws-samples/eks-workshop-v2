...
EKS 클러스터에 Fargate 프로파일을 추가하는 것부터 시작하겠습니다. 아래 명령은 다음 특성을 가진 `checkout-profile`이라는 Fargate 프로파일을 생성합니다:

1. `checkout` 네임스페이스에서 `fargate: yes` 레이블을 가진 Pod를 대상으로 함
2. Pod를 VPC의 프라이빗 서브넷에 배치
3. Fargate 인프라에 IAM 역할을 적용하여 ECR에서 이미지를 가져오고 CloudWatch에 로그를 작성하는 등의 작업을 수행할 수 있도록 함
