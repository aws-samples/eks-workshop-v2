...
EKS 클러스터에 Pod를 배포하고 AWS Inferentia용 샘플 모델을 컴파일하겠습니다. AWS Inferentia용 모델을 컴파일하려면 [AWS Neuron SDK](https://aws.amazon.com/machine-learning/neuron/)가 필요합니다. 이 SDK는 AWS에서 제공하는 [Deep Learning Containers (DLCs)](https://github.com/aws/deep-learning-containers/blob/v8.12-tf-1.15.5-tr-gpu-py37/available_images.md#neuron-inference-containers)에 포함되어 있습니다.

### Device Plugin 설치

DLC가 Neuron 코어를 사용하려면 이를 노출해야 합니다. [Neuron device plugin Kubernetes 매니페스트 파일](https://github.com/aws-neuron/aws-neuron-sdk/tree/master/src/k8)은 Neuron 코어를 DLC에 노출합니다. 이러한 매니페스트 파일은 EKS 클러스터에 사전 설치되어 있습니다.
...
1. `nodeSelector` 섹션에서 이 Pod를 실행할 인스턴스 타입을 지정합니다. 이 경우 trn1 인스턴스입니다.
2. `resources` `limits` 섹션에서 이 Pod를 실행하는 데 neuron 코어가 필요함을 지정합니다. 이는 Neuron Device Plugin에 neuron API를 Pod에 노출하도록 지시합니다.
