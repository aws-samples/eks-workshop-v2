...
locals {
  kubectl_context = "eks-workshop-auto"
}

#data "aws_iam_role" "auto_mode_node_role" {
#    name = "eksctl-eks-workshop-auto-cluster-AutoModeNodeRole-*"
#    filter {
#        name   = "tag:Name"
#        values = ["eksctl-eks-workshop-auto-cluster-AutoModeNodeRole-*"]
#    }
#}
#data "aws_eks_node_group" "default" {
#  cluster_name    = data.aws_eks_cluster.eks_cluster_auto.id
#  node_group_name = "default"
#}

data "aws_ssm_parameter" "eks_ami" {
  name = "/aws/service/eks/optimized-ami/${data.aws_eks_cluster.eks_cluster_auto.version}/amazon-linux-2023/x86_64/standard/recommended/image_id"
}
...
resource "null_resource" "kustomize_app1" {
  ...
  provisioner "local-exec" {
    command = "kubectl apply --context ${local.kubectl_context} -f - <<EOF\n${templatefile("${path.module}/deployment_permissions.yaml.tpl", {
...
resource "null_resource" "kustomize_app2" {
  ...
  provisioner "local-exec" {
    command = "kubectl apply --context ${local.kubectl_context} -f ${path.module}/deployment_image.yaml"
    when    = create
  }
}
