...
# The lab collects metrics into AMP by installing the amazon-cloudwatch-observability
# add-on and wiring an IAM role to the cloudwatch-agent service account via EKS Pod
# Identity. The EKS Pod Identity Agent add-on, the AMP workspace and Grafana are managed
# by Terraform and torn down by prepare-environment, so here we only remove what the lab
# steps created.
