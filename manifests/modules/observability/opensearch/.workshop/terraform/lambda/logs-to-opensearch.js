...
async function getOpenSearchHost() {
    const path = '/systemsmanager/parameters/get/?name=' + encodeURIComponent(process.env.OPENSEARCH_HOST_PARAMETER_PATH); 
    const requestParams = {
        hostname: 'localhost',
        port: 2773,
        path: path,
        ...
        const req = http.request(requestParams, res => {
          ...
          res.on('end', () => {
            ...
            try {
                ...
                console.log("Error retrieving OpenSearch host from SSM using path " + 
...
exports.handler = async function(input, context) {
    ...
    zlib.gunzip(zippedInput, function(error, buffer) {
        ...
        post(elasticsearchBulkData, endpoint, function(error, success, statusCode, failedItems) {
