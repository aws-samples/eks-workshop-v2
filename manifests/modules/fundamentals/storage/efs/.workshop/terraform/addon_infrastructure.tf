...
resource "aws_security_group" "efs" {
  ...
  tags = merge(
    local.tags,
    {
      Name = "${local.addon_context.eks_cluster_id}-efssecuritygroup"
    }
  )
}

resource "aws_efs_file_system" "efsassets" {
  creation_token = "${local.addon_context.eks_cluster_id}-efs-assets"
  encrypted      = true
  ...
  tags = merge(
    local.tags,
    {
      Name = "${local.addon_context.eks_cluster_id}-efs-assets"
    }
  )
}
...
resource "aws_kms_alias" "cmk_efs" {
  name          = "alias/${local.addon_context.eks_cluster_id}-cmk-efs"
