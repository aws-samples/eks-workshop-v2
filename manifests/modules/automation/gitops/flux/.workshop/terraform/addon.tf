...
resource "aws_codecommit_repository" "codecommit_retail_store_sample" {
  repository_name = "${local.addon_context.eks_cluster_id}-retail-store-sample"
...
resource "aws_iam_policy" "ci_access" {
  name   = "${local.addon_context.eks_cluster_id}-ci-access"
  path   = "/"
...
resource "aws_iam_role" "codepipeline_role" {
  name               = "${local.addon_context.eks_cluster_id}-codepipeline-role"
...
resource "aws_iam_role_policy" "codepipeline_policy" {
  name   = "${local.addon_context.eks_cluster_id}-codepipeline-policy"
...
resource "aws_iam_role" "codebuild_role" {
  name               = "${local.addon_context.eks_cluster_id}-codebuild-role"
...
module "iam_assumable_role_ui" {
  ...
  version                       = "~> v5.5.5"
  create_role                   = true
  role_name                     = "${local.addon_context.eks_cluster_id}-ecr-ui"
...
resource "aws_iam_policy" "ecr_ui" {
  name        = "${local.addon_context.eks_cluster_id}-ecr-ui"
  path        = "/"
